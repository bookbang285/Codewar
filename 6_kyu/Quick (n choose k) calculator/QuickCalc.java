...
public class QuickCalc {
    public BigInteger choose(int n, int p) { 
        return fac(n).divide(fac(p).multiply(fac(n - p)));
    }
  
    public BigInteger fac(int m){
        if(m>=1) return fac(m-1).multiply(BigInteger.valueOf(m));
