...
class CodeDecode {
    ...
    public static String code(String strng) {
        String []number = {"10","11","0110","0111","001100","001101","001110","001111","00011000","00011001"} ;
        strng = strng.replaceAll("1",number[1]);
        strng = strng.replaceAll("0",number[0]);
        for(int i =2;i<10;i++){
            strng = strng.replaceAll(String.valueOf(i),number[i]);
        }
        return strng;
    }
    public static String decode(String str) {
        ...
        for(int i =0 ; i <str.length();i++){
            for(int j =  0 ; j<number.length;j++){
                String st = str.substring(startAt,i+1);
                if(st.equals(number[j])){
                    s+=String.valueOf(j);
                    startAt = i+1;
                }
            }
